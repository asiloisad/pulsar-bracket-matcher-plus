...
module.exports = class BracketMatcher {
  constructor(editor, editorElement, matchManager) {
    ...
    this.subscriptions.add(
      atom.commands.add(
        editorElement,
        "bracket-matcher-plus:remove-brackets-from-selection",
        (event) => {
  ...
  insertText(text, options) {
    ...
    if (text === "#" && this.isCursorOnInterpolatedString()) {
      autoCompleteOpeningBracket =
        this.getScopedSetting("bracket-matcher-plus.autocompleteBrackets") &&
    ...
    if (this.isClosingBracket(text) && nextCharacter === text && !hasEscapeCharacterBeforeCursor) {
      ...
      if (bracketMarker || this.getScopedSetting("bracket-matcher-plus.alwaysSkipClosingPairs")) {
  ...
  wrapSelectionInBrackets(bracket) {
    ...
    if (!this.getScopedSetting("bracket-matcher-plus.wrapSelectionsInBrackets")) return false;
